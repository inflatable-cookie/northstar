...
5. Group work into meaningful **roadmap batches** (broad chunks), not
   agent-turn steps. Batch cards carry step-by-step detail in strict posture.
6. For non-trivial lanes, name at least two or three batches in `## Execution
   Plan` and anticipate several batch cards across the visible runway before
   execution starts.
7. Write `## Execution Plan`, `## Goals`, and `## Acceptance Criteria` as
   checkbox task lists (`- [ ]` / `- [x]`) so progress is scannable.
8. Make the lane runway explicit:
...
9. For the authoritative batch-card field list and expanded ready-state checks,
...
10. Apply the repo's readiness rubric before marking a card or short chain
   `ready`:
...
11. If the posture is `strict-paused`, compile toward a paused planning gate or
   intent checkpoint instead of pretending the lane is ready for execution.
12. Reference governing contract ids directly in each roadmap milestone.
13. Keep planning gaps visible in the milestone until they are closed.
14. Recommend generation rollover only when contract or architecture shifts make
...
15. Before recommending rollover, confirm all current-generation milestones can
...
16. Treat any remaining spec for the lane as context only; do not let it outrank
