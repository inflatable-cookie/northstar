...
- `Current State` captures:
  ...
  - for a worker handoff, the model capability profile selected under the
    ...
    recent-use rotation) and, when the worker is frontier, both escalation
    reasons (exceptional reasoning difficulty after planning, and highest
    priority or material consequence). Write `none` for
