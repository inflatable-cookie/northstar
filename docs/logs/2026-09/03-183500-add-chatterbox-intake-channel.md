...
Exact-file staging (`git add -- <exact-file>`), exact-path commit isolation
(`git commit -- <exact-file>`), and pre-stage index checks
(`git diff --cached --name-only`) ensure that pre-existing staged or dirty
files are never committed and remain untouched. This isolation is proven via an
executable temporary-repository fixture with 15 passing assertions.

Chatterbox v1 does not call `send_agent_prompt` or start an orchestrator turn
automatically. It reports the absolute note path and a one-line summary directly
to the operator in chat. The orchestrator inspects `docs/triage/` at normal
triage checkpoints and treats surfaced notes as non-assignments. A future atomic
queue or conditional-send adapter remains triage only. Chatterboxes refuse
implementation, promotion, review, merge, or worker dispatch.
...
| --- | --- | --- |
| `skills/northstar/references/modes/chatterbox.md` | Non-existent | Canonical mode for warm operator intake, problem identification, exact-path git commit isolation, operator note handoff (no turn start), and authority boundaries |
| `skills/northstar/commands/northstar-chatterbox/SKILL.md` | Non-existent | Thin explicit command adapter routing to chatterbox mode (675 bytes, 48-char description) |
...
| `docs/contracts/001-working-rules.md` | Roles table and delegate/continuation rules | Adds chatterbox role, shared checkout exact-path git isolation, and operator note intake handling |
...
| `bundle-docs/sections/07-delivery-framework-and-autonomy.md` | No chatterbox section | Adds `## Chatterbox intake channel` covering start, spawn, git isolation, notification boundary, and authority |
...
1. **Shared-Index Commit Isolation (`execution-miss`):**
   ...
   - Commit exact path: `git commit -m "docs(triage): ..." -- <exact-new-file>` (or `git commit --only -- <exact-new-file>`).
...
2. **Paseo Notification Boundary (`planning-change`):**
   - Settled on `origin/main` (`cac5122`): Paseo has no atomic notify-only or send-if-idle operation. Chatterbox v1 does not call `send_agent_prompt` or start an orchestrator turn.
   - Chatterbox v1 reports the absolute note path and one-line summary to the operator in chat.
   - The orchestrator inspects `docs/triage/` at normal checkpoints and treats surfaced notes as non-assignments.
   - Future atomic queue/conditional-send adapter remains triage only.
...
| 6. Capture does not interrupt live work | Chatterbox asked to notify orchestrator automatically | Mode omits `send_agent_prompt` / turn-starting calls; reports absolute path and summary to operator in chat. |
...
Orchestrator exact-head review of PR #31 and merge.
