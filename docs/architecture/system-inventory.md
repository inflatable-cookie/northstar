...
| Orchestrator coordination | Chatterbox-promoted commit + approved frontier | actual-worker dispatch, review routing, merge gate, merge, and closeout | spec 037, `001-working-rules` | Coordinator verifies factual prerequisites, launches the full approved frontier, reports, and yields; it does not promote or design parallelism |
...
| Independent review children | worker PR + clean exact-head worker-workspace lease + review oracle | provider verdict naming the exact reviewed head + coordinator merge gate | spec 037, `001-working-rules` | Reviewer is a visible parent-attached child in the worker workspace; no review-only workspace; same worker/reviewer on revisions when available; stale or ambiguous evidence stops merge |
