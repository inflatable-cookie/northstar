...
| Chatterbox planning and promotion | direct operator planning + delegate/external triage + explicit confirmation + coordinator pre-PR decision requests | canonical architecture/contracts/specs/roadmaps/cards, approved dispatch frontier, and cited ruling or operator-confirmed blocker direction | `001-working-rules`, `skills/northstar/references/modes/chatterbox.md` | Chatterbox promotes directly on `main`, disposes triage, rules only inside existing authority, converses with the operator for new intent, and does not implement or merge |
