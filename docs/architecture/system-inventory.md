...
| orchestrator thread | lane coordination and merge-gate surface | repo maintainers / operator | active Northstar lane | `skills/northstar/references/modes/orchestrator.md`, active spec/roadmap/contract |
...
| fresh orchestrator continuation | ownership transfer of one live planning/review lane | source yields; successor owns the lane | committed seven-section continuation handoff plus optional local workspace | `orchestrator-continuation` activation, capitalized `Orchestrator` label, manual pin when no native control |
...
| chatterbox thread | direct operator intake conversation and problem identification | operator + chatterbox | unique timestamped `docs/triage/` notes on shared checkout | `skills/northstar/references/modes/chatterbox.md`, `docs/specs/035-chatterbox-intake-channel.md`, exact-path git commit isolation |
...
| review child | independent exact-head PR review | reviewer child under orchestrator dispatch, in a dedicated PR-head workspace | durable provider verdict naming the exact reviewed head | `skills/northstar/references/modes/pr-review.md`, parent-attached child with finish notifications, coordination gate |
...
| Orchestrator planning | operator conversation + canonical planning spine | per-worker handoffs under `docs/handoffs/` | `026-orchestrator-thread-and-worker-pr-loop` | Questions settle intent before cards are marked ready. Parallel-first dispatch has no global thread budget; provider/profile failures stay lane-local |
| Conversational planning delegation | operator request + committed planning-delegate handoff + frontier profile | triage/research-only PR, then orchestrator promotion | `001-working-rules`, `026-orchestrator-thread-and-worker-pr-loop` | Delegate talks directly with operator; orchestrator reserves topic, reviews/merges, and separately promotes against current main |
...
| Chatterbox intake | direct operator start or orchestrator spawn + unique triage-file commit | unique `docs/triage/YYYYMMDD-HHMMSS-<slug>.md` notes on shared checkout | `035-chatterbox-intake-channel`, `001-working-rules` | Human intake chat; no worktree/branch/PR; exact-path git commit isolation; operator-visible note handoff; orchestrator treats intake as non-assignment; decision-ready notes separate confirmed from tentative meaning until the operator confirms through the orchestrator |
...
| Operator-confirmed promotion lane | decision-ready packet + explicit operator confirmation + exact promotion brief | canonical architecture/contracts/specs/roadmaps/cards changes on a branch/worktree PR | `036-economical-orchestrator-coordination`, `001-working-rules` | Bounded projection worker opens the PR and stops on semantic ambiguity; independent review child checks it against the packet; coordinator applies the merge gate |
| Independent review children | worker PR + dedicated PR-head workspace + review oracle | provider verdict naming the exact reviewed head + coordinator merge gate | `001-working-rules`, `026-orchestrator-thread-and-worker-pr-loop`, `036-economical-orchestrator-coordination` | Reviewer is a parent-attached child with finish notifications and an economical adequate route; same worker on revisions, same reviewer when available, replacement starts fresh; stale or ambiguous evidence stops merge |
