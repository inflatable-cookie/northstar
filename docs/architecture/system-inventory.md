...
| Orchestrator coordination | Chatterbox-promoted commit + approved frontier | actual-worker dispatch, review routing, merge gate, merge, closeout, and immediate next-ready progression | spec 037, `001-working-rules` | Coordinator continues across actionable boundaries without operator `continue`; yields for results/authority/empty runway; only empty runway notifies Chatterbox |
...
| Chatterbox planning and promotion | direct operator planning + delegate/external triage + explicit confirmation | canonical architecture/contracts/specs/roadmaps/cards and approved dispatch frontier | spec 037, `001-working-rules` | Chatterbox promotes directly on `main`, disposes triage, and sends typed direction to the coordinator; it does not implement or merge |
...
| Independent review children | worker PR + clean exact-head worker-workspace lease + review oracle + author provider/model identity | provider verdict naming the exact reviewed head + coordinator merge gate | spec 037, `001-working-rules` | Reviewer is a visible parent-attached child in the worker workspace and uses a different underlying provider/model from the worker; no review-only workspace; same reviewer on revisions when available; stale or ambiguous evidence stops merge |
