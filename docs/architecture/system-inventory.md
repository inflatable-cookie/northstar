...
| chatterbox thread | direct operator intake conversation and problem identification | operator + chatterbox | unique timestamped `docs/triage/` notes on shared checkout | `skills/northstar/references/modes/chatterbox.md`, `docs/specs/035-chatterbox-intake-channel.md`, idle-only Paseo ping |
...
| Chatterbox intake | direct operator start or orchestrator spawn + unique triage-file commit + idle-only Paseo ping | unique `docs/triage/YYYYMMDD-HHMMSS-<slug>.md` notes on shared checkout | `035-chatterbox-intake-channel`, `001-working-rules` | Human intake chat; no worktree/branch/PR; exact-file git add; running orchestrators skipped; orchestrator treats ping as intake only |
