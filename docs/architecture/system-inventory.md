...
| Orchestrator coordination | operator routing + canonical planning spine | per-worker handoffs under `docs/handoffs/` | `026-orchestrator-thread-and-worker-pr-loop`, `036-economical-orchestrator-coordination` | Operator-confirmed intent precedes promotion and ready cards. Parallel-first dispatch has no global thread budget; provider/profile failures stay lane-local |
| Conversational planning delegation | operator request + committed planning-delegate handoff + frontier profile | triage/research-only PR, then independent review child, coordinator merge, and operator-confirmed promotion | `001-working-rules`, `026-orchestrator-thread-and-worker-pr-loop` | Delegate talks directly with operator; the orchestrator reserves the topic, an independent review child reviews the PR, the coordinator merges after the gate, and an operator-confirmed lane promotes against current main |
