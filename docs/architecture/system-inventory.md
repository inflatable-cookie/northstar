...
| Chatterbox intake | direct operator start or orchestrator spawn + unique triage-file commit | unique `docs/triage/YYYYMMDD-HHMMSS-<slug>.md` notes on shared checkout | `035-chatterbox-intake-channel`, `001-working-rules` | Human intake chat; no worktree/branch/PR; exact-path git commit isolation; automated ping paused pending atomic queue; orchestrator treats intake as non-assignment |
