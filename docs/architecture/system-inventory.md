...
| orchestrator thread | mechanical dispatch, revision, review-routing, merge-gate, and closeout surface | repo maintainers / operator | Chatterbox-promoted canonical plan and approved frontier | `skills/northstar/references/modes/orchestrator.md`, `docs/contracts/001-working-rules.md`, active card/manifest |
...
| Conversational planning delegation | operator request or Chatterbox start for one bounded topic | unique triage note and administrative notice to Chatterbox | `001-working-rules`, `skills/northstar/references/modes/chatterbox.md` | Same-workspace delegate talks directly with the operator, may use read-only research, and has no canonical/PR/coordinator authority |
| Fresh orchestrator continuation | operator request + committed continuation handoff + optional Paseo local workspace | successor orchestrator owns the transferred lane | `001-working-rules`, `skills/northstar/references/modes/orchestrator.md` | Source yields after pushed dispatch; successor is not a worker; pin placement is manual without a native control |
| Chatterbox planning and promotion | direct operator planning + delegate/external triage + explicit confirmation + coordinator pre-PR decision requests | canonical architecture/contracts/specs/roadmaps/tasks, approved dispatch frontier, cited ruling or operator-confirmed blocker direction, and explicitly authorized small direct changes | `001-working-rules`, `skills/northstar/references/modes/chatterbox.md` | Chatterbox promotes directly on `main`, disposes triage, rules only inside existing authority, and may make one named local low-risk change under the complete operator-authorized gate; it never reviews or merges |
| Mechanical documentation projection (non-semantic helper) | settled orchestrator brief + captured dirty state + allowed paths | named documentation changes and deterministic validation report in the planning checkout | `001-working-rules`, `skills/northstar/references/modes/orchestrator.md` | Fast/low-cost, serial, genuinely non-semantic only, no Git/provider authority; orchestrator reviews the full diff before any Git mutation |
