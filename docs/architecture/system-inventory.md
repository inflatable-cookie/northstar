...
| `template-bundle/` | product artifact | repo maintainers | copy-ready downstream docs bundle | `docs/contracts/001-working-rules.md`, `docs/roadmaps/archive/g01.md` |
...
| language quality packs | automation surface | repo maintainers | optional language-specific authoring and audit workflows | source translation memos, `docs/contracts/004-language-quality-pack.md`, official registry, Effigy selectors; Rust v2 recorder boundary promoted from live evidence |
...
| `docs/handoffs/` | fresh-thread coordination surface | handoff/Coordinator mode | timestamped takeover notes | handoff contract and templates |
| `docs/triage/` | mutable temporary conversational capture surface | Chatterbox/frontier-advisor/refresh/cleanup modes | timestamped non-authoritative notes kept current in place and pruned on promotion | triage contract, refresh lifecycle, cleanup route |
...
| Coordinator thread | mechanical dispatch, revision, review-routing, merge-gate, and closeout surface | repo maintainers / operator | Chatterbox-promoted canonical plan and approved frontier | `skills/northstar/references/modes/coordinator.md`, `docs/contracts/001-working-rules.md`, active card/manifest |
| Frontier second opinion | optional planning and evidence challenge, created only after explicit operator approval | operator + host advisor/committee, under Chatterbox planning authority | bounded advisory intake or one unique triage note | no Northstar role, no automatic launch, redacted minimum evidence, return to Chatterbox |
| fresh Coordinator continuation | ownership transfer of one live coordination lane | source yields; successor owns the lane | committed seven-section continuation handoff plus optional local workspace | `coordinator-continuation` activation, capitalized `Coordinator` label, manual pin when no native control |
| fresh Chatterbox continuation | ownership transfer of one live planning conversation | source yields after verified transfer; successor owns the lane | committed seven-section handoff, same-workspace child tab, and optional Queue attention reassignment | `chatterbox-continuation` activation, `Chatterbox=true`, exact task set, source retained |
| documentation projection subagent | non-semantic mechanical doc projection | Coordinator | same-checkout materialization of already-settled, non-semantic edits from a settled brief; stops on ambiguity and carries no new product meaning | Coordinator projection template, Coordinator full-diff review, no Git/provider authority |
...
| Coordinator coordination | Chatterbox-promoted commit + approved frontier | actual-worker dispatch, pre-PR decision requests to Chatterbox, review routing, merge gate, merge, closeout, and immediate next-ready progression | `001-working-rules`, `skills/northstar/references/modes/coordinator.md` | Coordinator continues across actionable boundaries without operator `continue`; yields for results/authority/empty runway; ordinary child waiting is silent, while empty runway and pre-PR semantic blockers notify Chatterbox |
| Frontier second opinion | explicit operator approval after a request or Chatterbox recommendation | minimal redacted intake returned to Chatterbox | `001-working-rules`, `skills/northstar/references/modes/chatterbox.md` | uses the host advisor/committee mechanism; no automatic launch or Northstar role |
| Fresh Coordinator continuation | operator request + committed continuation handoff + optional Paseo local workspace | successor Coordinator owns the transferred lane | `001-working-rules`, `skills/northstar/references/modes/coordinator.md` | Source yields after pushed dispatch; successor is not a worker; pin placement is manual without a native control |
| Chatterbox planning and promotion | direct operator planning + optional operator-approved advisor intake + external triage + explicit confirmation + Coordinator pre-PR decision requests | canonical architecture/contracts/specs/roadmaps/tasks, approved dispatch frontier, cited ruling or operator-confirmed blocker direction, and explicitly authorized small direct changes | `001-working-rules`, `skills/northstar/references/modes/chatterbox.md` | Chatterbox promotes directly on `main`, disposes triage, rules only inside existing authority, and may make one named local low-risk change under the complete operator-authorized gate; it never reviews or merges |
| Mechanical documentation projection (non-semantic helper) | settled Coordinator brief + captured dirty state + allowed paths | named documentation changes and deterministic validation report in the planning checkout | `001-working-rules`, `skills/northstar/references/modes/coordinator.md` | Fast/low-cost, serial, genuinely non-semantic only, no Git/provider authority; Coordinator reviews the full diff before any Git mutation |
| Operator-confirmed planning promotion | Chatterbox-reconciled meaning + explicit operator confirmation | canonical ready plan committed directly on integration branch | `001-working-rules`, `skills/northstar/references/modes/chatterbox.md` | No promotion worker; Chatterbox may recommend independent planning review, but it runs only after explicit operator approval |
...
| Coordinator mode | per-worker committed handoff under `docs/handoffs/`, fresh worker worktree or safe temporary fallback, startup check, chunk report, PR, provider review record, and closeout evidence | repo maintainers + operator | operator validation complete; longer multi-card evidence pending |
