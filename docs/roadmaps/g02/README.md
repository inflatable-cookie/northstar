...
| Raise language-code quality through evidence-bound routed workflows without making every implementation part of the root payload. | active | `docs/contracts/004-language-quality-pack.md`, `docs/architecture/system-architecture.md`, `docs/roadmaps/g02/048-extract-modular-language-quality-packages.md` | card 117 blocked on planning decisions (findings 1-2); card 118 planned |
...
- `048-extract-modular-language-quality-packages.md` (active; card 117 blocked on
  planning decisions (findings 1-2); cards 118-120 planned in dependency order)

## Next Task

`g02.048/117` proved the generic lifecycle (byte-exact digest vectors,
operator trust and lifecycle state, atomic CAS, identity-bound routing,
transactional acquire/update/rollback, offline routing, revocable trust) with
all eight review-oracle rows falsified. Its PR 22 is open; round-2 review
accepted the byte-level repair and returned two planning-change findings
(portable core host/API; self-check runner contract). The card is blocked on
those decisions; cards 118-120 remain planned behind them.
