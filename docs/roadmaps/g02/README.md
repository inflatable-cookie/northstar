...
| Keep the orchestrator on the main runway by giving the operator independent chatterbox threads for side issues and ideas. | planned | `docs/specs/035-chatterbox-intake-channel.md`, `docs/specs/026-orchestrator-thread-and-worker-pr-loop.md`, `docs/specs/030-conversational-triage-and-docs-cleanup.md` | `g02.050` after card 120 |
...
- `049-preserve-paseo-worker-parentage.md` (complete; card 123 preserves
  ...
  same-child revision)
- `050-add-chatterbox-intake-channel.md` (compiled; card 124 is not ready —
  blocked on card 120's skill/router/command ownership)

## Next Task

Orchestrator: exact-head review of the card-120 PR against `main`, merge, and
refresh the installed skill (source payload is now 111 files). Spec 035 /
`g02.050` / card 124 (chatterbox intake) is compiled and stays serial behind
the skill/router/command ownership that card 120's removal transfers; the
operator checkpoint that follows owns the Sentrux triage note and any
new-language cohort.
