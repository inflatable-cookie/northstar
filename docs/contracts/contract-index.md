...
`g02.027/079` is complete; its later consumer measurement remains pending. The
generation remains open and no separate contract gap blocks the current planned
dogfood, but the live Poodle destination must be selected from Poodle's current
generation index before dispatch.
